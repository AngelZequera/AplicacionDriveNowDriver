...
import com.aztec.dradomiconductor.databinding.ActivityMapTripBinding
import com.aztec.dradomiconductor.databinding.ModalBottonSheetTripInfoBinding
import com.aztec.dradomiconductor.fragments.ModalBottonSheetBooking
import com.aztec.dradomiconductor.fragments.ModalBottonSheetTripInfo
...
import com.aztec.dradomiconductor.models.Price
...
import com.aztec.dradomiconductor.providers.ConfigProvider
...
class MapTripActivity : AppCompatActivity(), OnMapReadyCallback,  Listener, DirectionUtil.DirectionCallBack, SensorEventListener{
    ...
    private  var km = 0.0
    //Guadar la localizacion anterior
    ...
    private var runnable = Runnable {
        kotlin.run {
            ...
            if (min == 0) {
                binding.txtTimer.text = "$counter Seg"
            }
            else {
                binding.txtTimer.text = "$min Min $counter Seg"
            }
            ...
            override fun onResponse(call: Call<FCMResponse>, response: Response<FCMResponse>) {
                if (response.body() != null){
                    if (response.body()!!.succes == 1){
                        ...
                        Toast.makeText(this@MapTripActivity, "No se puedo enviar la notificación", Toast.LENGTH_LONG).show()
                    }
                }else{
                    Toast.makeText(this@MapTripActivity, "Error al enviar notificación", Toast.LENGTH_LONG).show()
                }
            }
  ...
  private fun addDestinationMarker(){
    ...
    private fun saveLocation() {
        if (myLocationLating != null) {
            geoProvider.saveLocationWorking(authProvider.getId(), myLocationLating!!)
        }
    }
    ...
    private fun showButtomFinish(){
        binding.btnStartTrip.visibility = View.GONE  // Ocultando el boton de iniciar
        binding.btnFinishTrip.visibility = View.VISIBLE // Mostrando el boton de terminar
    }
    ...
    private fun updateToStarted(){
        if(isCloseToOrigin){
            bookingProvider.updateStatus(booking?.idCliente!!, "started").addOnCompleteListener {
                if(it.isSuccessful){
                    if(destinationLatLng != null){
                        isStartedTrip = true
                        googleMap?.clear()
                        ...
                        startTimer()
                        sendNotification("Servicio medico comenzando")
                    }
                    showButtomFinish()
                }
            }
    ...
    private fun createHistory(){
        ...
        historyProvider.create(history).addOnCompleteListener {
            if(it.isSuccessful){
                bookingProvider.updateStatus(booking?.idCliente!!, "finished").addOnCompleteListener {
                    if(it.isSuccessful){
                        goToCalificationClient()
                        sendNotification("Servicio medico finalizado")
                    }
                }
            }
        }
    ...
    private fun getPrices(distance: Double, time: Double){
        configProvider.getPrice().addOnSuccessListener { document ->
            if(document.exists()){
                ...
                totalPrice = if(totalPrice < 100.0) prices?.minValue!! else totalPrice
                createHistory()
            }
        }
    }
    ...
    override fun currentLocation(location: Location) { // Actualizacion de la posicion en tiempo real
        ...
        if(isStartedTrip){
            ...
            //Medir en kilometros
            km = meters / 1000
            binding.txtDistance.text = "${String.format("%.1f",km)} km"
        }
...
//        ))
        ...
        if(booking != null && originLatLng != null){
            var distance = getDistanceBetween(myLocationLating!!, originLatLng!!)
            if(distance <= 300){
